...
app.get('/posts/:slug', (req, res) => {
	...
	if (index < 0) {
		let slicedSlug = req.params.slug.slice(
			0,
			req.params.slug.lastIndexOf('.')
		)
	...
	res.render('index', {
		...
		content: { include: 'post', data: postData },
	})
})
...
app.get('/feed.rss', (req, res) => {
	...
	for (var post of posts) {
		postsExtended.push(
			Object.assign(post, {
				body: helpers.cachePostContents(post.file, cache),
			})
		)
	}
...
app.get('*.html', (req, res) => {
	let realPath = req.path.slice(0, -5)
	res.redirect(301, realPath)
})
